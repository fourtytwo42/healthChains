...
2. **Backend Layer**:
   ...
   - **Structured Logging**: Winston logger with file output and log levels
   - **Error Handling**: Centralized error handling with structured logging
   - **Memory Optimization**: Batch processing for events (50 events per batch)
   - **Rate Limiting**: API rate limiting (10,000 requests/15min general, 2,000/15min for expensive endpoints)
   - **Request Timeout**: 30 second timeout for all requests to prevent resource exhaustion
   - **RPC Health Checks**: Periodic health checks with automatic reconnection
